            ...
            <section className="pt-32 pb-16 px-6 lg:px-12 bg-surface border-b border-border">
                <div className="max-w-4xl mx-auto text-center">
                    ...
                    <p className="text-xl text-textSecondary max-w-2xl mx-auto">
                        Our journey to build the most powerful treasury automation protocol on Bitcoin Cash
                    </p>
                </div>
            </section>
                ...
                <div className="max-w-6xl mx-auto">
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <div className="font-semibold text-textPrimary">Current Phase: Beta & Security Hardening</div>
                                <div className="text-sm text-textSecondary">Preparing for mainnet launch</div>
                            </div>
                        </div>
                        <Link
                            to="/changelog"
                                ...
                                <div key={phase.phase} className="relative">
                                    ...
                                    <div className="flex items-start gap-6 mb-8">
                                        <div className="flex-shrink-0">
                                            <div className={`w-16 h-16 rounded-2xl ${STATUS_CONFIG[phase.status].bg} border ${STATUS_CONFIG[phase.status].border} flex items-center justify-center`}>
                                        ...
                                        <div className="flex-1">
                                            <div className="flex flex-wrap items-center gap-3 mb-2">
                                                ...
                                                <span className={`px-3 py-1 text-xs font-semibold rounded-full border ${STATUS_CONFIG[phase.status].bg} ${STATUS_CONFIG[phase.status].color} ${STATUS_CONFIG[phase.status].border}`}>
                                        ...
                                        {phase.items.map((item, itemIndex) => {
                                            ...
                                            return (
                                                <div key={itemIndex} className="relative">
                                                    {/* Timeline dot */}
                                                    <div className={`absolute -left-[37px] top-6 w-4 h-4 rounded-full border-2 ${STATUS_CONFIG[item.status].bg} ${STATUS_CONFIG[item.status].border}`} />

                                                    <div className="bg-surface border border-border rounded-xl p-6 hover:border-primary/30 transition-colors">
                                                        <div className="flex items-start gap-4 mb-4">
                                                            <div className={`flex-shrink-0 w-10 h-10 rounded-lg ${STATUS_CONFIG[item.status].bg} border ${STATUS_CONFIG[item.status].border} flex items-center justify-center`}>
                                                                <ItemIcon className={`w-5 h-5 ${STATUS_CONFIG[item.status].color}`} />
                                                            </div>
                                                            <div className="flex-1">
                                                                ...
                                                                <ul className="space-y-2">
                                                                    {item.items.map((subItem, subIndex) => (
                                                                        <li key={subIndex} className="flex items-start gap-2 text-sm text-textSecondary">
                                                                            <CheckCircle2 className={`w-4 h-4 mt-0.5 flex-shrink-0 ${item.status === 'completed' ? 'text-success' : 'text-textMuted'}`} />
                                                                            <span>{subItem}</span>
                                                                        </li>
                                                                    ))}
                                                                </ul>
            ...
            <section className="py-16 px-6 lg:px-12 bg-surface border-t border-border">
                <div className="max-w-4xl mx-auto text-center">
                    ...
                    <p className="text-lg text-textSecondary mb-8 max-w-2xl mx-auto">
                        Join our community to provide feedback, request features, and help build the future of treasury automation on Bitcoin Cash.
                    </p>
