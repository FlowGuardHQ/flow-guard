...
<Steps>
  ...
  <Step title="Generate merkle tree (KYC only)">
    If KYC is enabled, upload a CSV with `address,amount` rows. The app POSTs to `POST /api/airdrops/:id/generate-merkle` and stores the merkle root in the contract.
  </Step>
